...
# ex 1: $n + $d within one paren group
{
    ...
    my $r = _decorate( '111', 'postfix', a => 'Brussels Hemoglobin Symposium', n => '1st', d => '1983' );
    is( combined($r), 'Brussels Hemoglobin Symposium (1st : 1983)', '111 ex1: combined' );
    is( $r->[1], 'Brussels Hemoglobin Symposium', '111 ex1: $a unchanged' );
    is( $r->[3], ' (1st : ', '111 ex1: $n opens group (lead space) + " : " (nd)' );
    is( $r->[5], '1983)', '111 ex1: $d closes group' );
    check_combined( '111', '111 ex1: $a+$n+$d', a => 'Brussels Hemoglobin Symposium', n => '1st', d => '1983' ) }

# ex 2: $g qualifier + $d/$c group
{
    # render: 111 2# $a Governor's Conference on Aging $g N.Y. $d 1982 $c Albany, N.Y.
    my $r = _decorate( '111', 'postfix', a => "Governor's Conference on Aging", g => 'N.Y.', d => '1982', c => 'Albany, N.Y.' );
    is( combined($r), "Governor's Conference on Aging (N.Y.) (1982 : Albany, N.Y.)", '111 ex2: combined' );
    ...
    check_combined( '111', '111 ex2: $a+$g+$d+$c', a => "Governor's Conference on Aging", g => 'N.Y.', d => '1982', c => 'Albany, N.Y.' ) }

# ex 4: $g + $n/$d/$c full group
{
    # render: 111 2# $a Military History Symposium $g U.S. $n 9th $d 1980 $c United States Air Force Academy
    my $r = _decorate( '111', 'postfix', a => 'Military History Symposium', g => 'U.S.', n => '9th', d => '1980', c => 'United States Air Force Academy' );
    is( combined($r), 'Military History Symposium (U.S.) (9th : 1980 : United States Air Force Academy)', '111 ex4: combined' );
    is( $r->[3], ' (U.S.)', '111 ex4: $g wrapped (lead space)' );
    is( $r->[5], ' (9th : ', '111 ex4: $n opens group + " : " (nd)' );
    is( $r->[7], '1980 : ', '111 ex4: $d gets " : " (dc)' );
    is( $r->[9], 'United States Air Force Academy)', '111 ex4: $c closes group' );
    check_combined( '111', '111 ex4: $a+$g+$n+$d+$c', a => 'Military History Symposium', g => 'U.S.', n => '9th', d => '1980', c => 'United States Air Force Academy' ) }
...
# $e subordinate unit after a $n/$d/$c group (uses '. ' per meeting spec)
{
    ...
    my $r = _decorate( '111', 'postfix', a => 'Olympic Games', n => '21st', d => '1976', c => 'Montréal, Québec', e => 'Organizing Committee', e => 'Arts and Culture Program', e => 'Visual Arts Section' );
    is( combined($r), 'Olympic Games (21st : 1976 : Montréal, Québec). Organizing Committee. Arts and Culture Program. Visual Arts Section', '111: $e chain after group' );
    is( $r->[3], ' (21st : ', '111: $n opens group (lead space)' );
    is( $r->[5], '1976 : ', '111: $d gets " : " (dc)' );
    is( $r->[7], 'Montréal, Québec). ', '111: $c closes group + ". " for $e' );
    is( $r->[9], 'Organizing Committee. ', '111: first $e gets ". " for next $e' );
    is( $r->[11], 'Arts and Culture Program. ', '111: second $e gets ". " for next $e' );
    is( $r->[13], 'Visual Arts Section', '111: last $e unchanged' );
    check_combined( '111', '111: $a+$n+$d+$c+3x$e', a => 'Olympic Games', n => '21st', d => '1976', c => 'Montréal, Québec', e => 'Organizing Committee', e => 'Arts and Culture Program', e => 'Visual Arts Section' ) }

# $q name after jurisdiction -> '. '
{
    # render: 111 2# $a Boston $q a city on a hill $d 1980 $c Boston, Mass.
    my $r = _decorate( '111', 'postfix', a => 'Boston', q => 'a city on a hill', d => '1980', c => 'Boston, Mass.' );
    is( combined($r), 'Boston. a city on a hill (1980 : Boston, Mass.)', '111: $q gets ". " then group' );
    ...
    check_combined( '111', '111: $a+$q+$d+$c', a => 'Boston', q => 'a city on a hill', d => '1980', c => 'Boston, Mass.' ) }

# $j relator term -> ', '
{
    # render: 111 2# $a Canadian Meteorological Society $g Canada $j issuing body
    my $r = _decorate( '111', 'postfix', a => 'Canadian Meteorological Society', g => 'Canada', j => 'issuing body' );
    is( combined($r), 'Canadian Meteorological Society (Canada), issuing body', '111: $j relator' );
    is( $r->[3], ' (Canada), ', '111: $g wrapped + $j next ", "' );
    is( $r->[5], 'issuing body', '111: $j last' );
    check_combined( '111', '111: $a+$g+$j', a => 'Canadian Meteorological Society', g => 'Canada', j => 'issuing body' ) }

# multiple $c locations joined by ' ; ' (cc)
{
    # render: 111 2# $a Meeting $d 1983 $c Berlin $c Leipzig
    my $r = _decorate( '111', 'postfix', a => 'Meeting', d => '1983', c => 'Berlin', c => 'Leipzig' );
    is( combined($r), 'Meeting (1983 : Berlin ; Leipzig)', '111: multiple $c use " ; " (cc)' );
    check_combined( '111', '111: $a+$d+$c+$c', a => 'Meeting', d => '1983', c => 'Berlin', c => 'Leipzig' ) }

# lone $n -> wrapped by group callback
{
    # render: 111 2# $a Symposium $n 1st
    my $r = _decorate( '111', 'postfix', a => 'Symposium', n => '1st' );
    is( combined($r), 'Symposium (1st)', '111: lone $n wrapped' );
...
# ===== 711 (Added Entry - Meeting Name) - aliases 111 =====
{
    ...
    my $r = _decorate( '711', 'postfix', a => 'Theatertreffen Berlin', g => 'Festival' );
    is( combined($r), 'Theatertreffen Berlin (Festival)', '711 ex3: $g wrapped' );
    is( $r->[3], ' (Festival)', '711 ex3: $g wrapped (lead space)' );
    check_combined( '711', '711 ex3: $a+$g', a => 'Theatertreffen Berlin', g => 'Festival' ) }

{
    # render: 711 2# $a Olympic Games $n 21st $d 1976 $c Montréal, Québec $e Organizing Committee
    my $r = _decorate( '711', 'postfix', a => 'Olympic Games', n => '21st', d => '1976', c => 'Montréal, Québec', e => 'Organizing Committee' );
    is( combined($r), 'Olympic Games (21st : 1976 : Montréal, Québec). Organizing Committee', '711: group + $e' );
    check_combined( '711', '711: $a+$n+$d+$c+$e', a => 'Olympic Games', n => '21st', d => '1976', c => 'Montréal, Québec', e => 'Organizing Committee' ) }

# ===== 611 (Subject Added Entry - Meeting Name) =====
# subdivisions $v/$x/$y/$z are NOT punctuated
{
    # render: 611 2# $a Meeting $d 1980 $c Berlin $x History $v Periodicals
    my $r = _decorate( '611', 'postfix', a => 'Meeting', d => '1980', c => 'Berlin', x => 'History', v => 'Periodicals' );
    is( combined($r), 'Meeting (1980 : Berlin)HistoryPeriodicals', '611: subdivisions no punct' );
    ...
    check_combined( '611', '611: $a+$d+$c+$x+$v', a => 'Meeting', d => '1980', c => 'Berlin', x => 'History', v => 'Periodicals' ) }

# ===== 811 (Series Added Entry - Meeting Name) =====
# $v (volume) IS punctuated with ' ;'
{
    # render: 811 2# $a Symposium $n 5th $d 1990 $c Paris $v no. 3
    my $r = _decorate( '811', 'postfix', a => 'Symposium', n => '5th', d => '1990', c => 'Paris', v => 'no. 3' );
    is( combined($r), 'Symposium (5th : 1990 : Paris) ;no. 3', '811: group + $v " ;"' );
    ...
    check_combined( '811', '811: $a+$n+$d+$c+$v', a => 'Symposium', n => '5th', d => '1990', c => 'Paris', v => 'no. 3' ) }
